...
也可以在設定裡製作短 GIF 小片段，適合保存幾秒鐘的畫面變化。
...
- 可製作 GIF 小片段，長度 `1` 到 `10` 秒，預設 `5` 秒。
- GIF 每秒擷取 `2` 張目前可見畫面，不包含聲音。
...
2. 點 Chrome 工具列上的「動畫瘋截圖小工具」圖示。
3. 點右上角小齒輪。
4. 在「GIF 小片段」區塊調整長度與解析度。
5. 按「開始製作 GIF」。
6. Popup 會顯示目前進度，例如 `製作中 4/10`。
7. 完成後，頁面右上角會顯示「GIF 製作完成」，檔案會下載到設定的資料夾。

GIF 製作期間請讓動畫瘋頁面保持在目前視窗中，因為它擷取的是 Chrome 當下可見畫面。
...
GIF 長度可以用滑桿設定 `1` 到 `10` 秒。

解析度可以選 `720P`、`1080P`、`1440P`。
...
Chrome 的 `captureVisibleTab()` 有呼叫頻率限制，所以這個工具用每秒 2 張畫面的方式製作 GIF。

它適合保存短片段，不適合當成完整錄影工具。
