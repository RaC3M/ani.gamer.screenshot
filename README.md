...
GIF 製作期間工具會短暫移動播放器時間軸來擷取影格，完成後會盡量回到原本播放位置。
...
### 為什麼 GIF 製作時播放器時間軸會移動？

GIF 會用 `video + canvas` 逐格擷取影片畫面。為了取得指定時間範圍內的影格，工具會短暫 seek 到不同時間點，完成後會盡量回到原本位置。

### 為什麼 GIF 還是可能失敗？

如果網站、瀏覽器或影片來源限制 canvas 讀取影片畫面，可能會導致影格擷取失敗。這種情況工具只會顯示錯誤，不會嘗試繞過限制。
